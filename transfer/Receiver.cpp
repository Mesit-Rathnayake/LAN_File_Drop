...
void receiveSingleFile(socket_t client, const std::string &destDir, size_t fileIndex, size_t totalFiles, const std::string& senderIP) {
    uint64_t name_len = 0;
    if (!recvAllBytes(client, reinterpret_cast<char *>(&name_len), sizeof(name_len))) return;

    std::string filename(static_cast<size_t>(name_len), '\0');
    if (!recvAllBytes(client, &filename[0], static_cast<size_t>(name_len))) return;

    std::cout << "[" << (fileIndex + 1) << "/" << totalFiles << "] Receiving: " << filename << "\n";

    uint64_t filesize = 0;
    if (!recvAllBytes(client, reinterpret_cast<char *>(&filesize), sizeof(filesize))) return;
    ...
    try {
        ...
        while (received < filesize) {
            ...
            int bytes_to_read = (remaining < sizeof(buffer)) ? static_cast<int>(remaining) : sizeof(buffer);
            int recv_bytes = recv(client, buffer, bytes_to_read, 0);
        ...
        std::cout << "\n";
        file.close();
        HistoryManager::logTransfer(outPath.string(), "Received", senderIP);
    } catch (...) {}
}
...
void receiveFile(int port = 9999, const std::string &destDir = ".") {
    ...
    if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) return;
    listen(sock, 1);
    ...
    std::string senderIPStr(clientIP);

    uint64_t num_files = 0;
    if (recv(client, reinterpret_cast<char *>(&num_files), sizeof(num_files), 0) > 0) {
        for (size_t i = 0; i < static_cast<size_t>(num_files); ++i) {
            receiveSingleFile(client, destDir, i, static_cast<size_t>(num_files), senderIPStr);
        }
    }
    CLOSE_SOCKET(client); CLOSE_SOCKET(sock);
}